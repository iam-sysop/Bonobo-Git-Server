...
namespace gitserverdotnet.Test.MembershipTests.EFTests
{
    class SqlServerTestConnection : IDatabaseTestConnection
    {
        ...
        public SqlServerTestConnection()
        {
            ...
            var fileName = Path.Combine(Path.GetTempPath(), "gitserverdotnetTestDb_" + _databaseName + ".mdf");
            CreateDB(fileName);
        ...
        {
            foreach (var dbFile in Directory.EnumerateFiles(Path.GetTempPath(), "gitserverdotnetTestDb_*"))
            {
                try
                {
                    File.Delete(dbFile);
